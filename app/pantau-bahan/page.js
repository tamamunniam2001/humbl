            ...
            {STATUSES.map(s => {
              ...
              return (
                <div key={s.key} className="card" style={{ overflow: 'hidden' }}>
                  {/* Header section */}
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '8px', background: s.bg }}>
                  ...
                  {/* Daftar item */}
                  <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px', minHeight: '90px' }}>
                    {list.length === 0 ? (
                      <div style={{ padding: '30px 12px', textAlign: 'center', fontSize: '12px', color: 'var(--muted)' }}>
                        {loading ? 'Memuat...' : (q ? 'Tidak ada item cocok' : s.empty)}
                      </div>
                    ) : list.map(item => (
                      <div key={item.id} style={{ border: '1px solid var(--border)', borderRadius: '11px', padding: '10px 12px', background: 'var(--surface2)', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                        {/* Info item */}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '5px' }}>
                            ...
                            <span className="badge badge-gray" style={{ fontSize: '10px' }}>{fmt(item.qty)} {item.satuan}</span>
                          </div>
