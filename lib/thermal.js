...
function buildReceipt(tx, settings = DEFAULT_SETTINGS) {
  ...
  hr()
  if (tx.discountAmount > 0 || tx.taxAmount > 0) {
    const sub = tx.subtotal ?? (tx.total - (tx.taxAmount ?? 0) + (tx.discountAmount ?? 0))
    row('Subtotal', `Rp ${fmt(sub)}`)
    if (tx.discountAmount > 0) row(`Diskon (${tx.discount}%)`, `- Rp ${fmt(tx.discountAmount)}`)
    if (tx.taxAmount > 0) row(`Pajak (${tx.tax}%)`, `+ Rp ${fmt(tx.taxAmount)}`)
  }
