...
async function printThermal(tx) {
  ...
  // Struk dapur (jika ada item)
  if (kitchenItems.length > 0) {
    await new Promise(r => setTimeout(r, 300))
    await send(buildStationReceipt(tx, 'DAPUR', kitchenItems, settings))
  }

  // Struk bar (jika ada item)
  if (barItems.length > 0) {
    await new Promise(r => setTimeout(r, 300))
    await send(buildStationReceipt(tx, 'BAR', barItems, settings))
  }
