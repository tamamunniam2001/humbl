...
async function printThermal(tx) {
  ...
  await send(buildReceipt(tx, settings))
  if (kitchenItems.length > 0) { await new Promise(r => setTimeout(r, 300)); await send(buildStationReceipt(tx, 'DAPUR', kitchenItems, settings)) }
  if (barItems.length > 0) { await new Promise(r => setTimeout(r, 300)); await send(buildStationReceipt(tx, 'BAR', barItems, settings)) }
...
export { printThermal, printKitchen, printBar, buildReceipt, disconnectPrinter, resetSettingsCache, connectPrinter, getPrinterStatus }
